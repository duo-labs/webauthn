...
func (p *ParsedCredentialAssertionData) Verify(storedChallenge string, relyingPartyID, relyingPartyOrigin, u2fAppID string, verifyUser bool, credentialBytes []byte) error {
	...
	if u2fAppID != "" {
		appIDHash = sha256.Sum256([]byte(u2fAppID))
	}
